...
//cd /mnt/c/Users/conweg/Documents/Coding/Advent_of_Code_2021/Day15/Puzzle2
...
int main(int argc, char const *argv[]) {
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < (int) map.size(); j++) {
      std::vector<int> temp;
      for (int k = (int) map[j].size() - orig; k < (int) map[j].size(); k++) {
        int num;
        if (map[j][k] + 1 > 9) {
          num = 1;
        } else {
      ...
      map[j].insert(map[j].end(), temp.begin(), temp.end());
    }
  }
  for (int i = 0; i < 4; i++) {
    int orig2  = map.size();
    for (int j = orig2 - orig; j < orig2; j++) {
