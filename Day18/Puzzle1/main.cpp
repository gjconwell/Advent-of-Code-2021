...
// cd /mnt/c/Users/gjcon/github/Advent-of-Code-2021/Day18/Puzzle1
...
int main(int argc, char const *argv[]) {
  ...
  while (istr >> info) {
    ...
    std::vector<bool> temp;
    Parse(tree2, true, info, 0, 1, 1, temp);
    ...
    while (edit) {
      std::vector<bool> xpath;
      bool splode = false;
      DetectExplode(tree1, 0, splode, temp, xpath);
      std::vector<bool> spath;
      bool splt = false;
      DetectSplit(tree1, splt, temp, spath);
      if (splode) {
        Explode(tree1, xpath);
      } else if (splt) {
        Split(tree1, spath);
