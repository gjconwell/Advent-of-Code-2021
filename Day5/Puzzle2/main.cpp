...
//cd /mnt/c/Users/conweg/Documents/Coding/Advent_of_Code_2021/Day5/Puzzle2
//g++ main.cpp -o main.exe -Wall
//./main.exe

void AddDiagonal(std::vector<std::pair<int, int>> &pairvec, int x1, int x2, 
       int y1, int y2) {
  if (x1 < x2 && y1 < y2) {
    int j = y1;
    for (int i = x1; i < x2 + 1; i++) {
      pairvec.push_back(std::make_pair(i, j));
      j++;
    }
  } else if (x1 > x2 && y1 > y2) {
    int j = y1;
...
void AddVec(std::vector<std::pair<int, int>> &pairvec, int x1, int x2, int y1, int y2) {
  if (x1 == x2 || y1 == y2) {
    ...
    AddDiagonal(pairvec, x1, x2, y1, y2);
  }
}
